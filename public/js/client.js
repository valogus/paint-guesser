...
// достаем верхнюю панель в конце игры и выводим лучшего игрока!
  if (event.data === 'open'){
    ...
    if(bestScore, bestPlayer){
        messageContainer.innerHTML += `Победитель:${bestPlayer.slice(0,bestPlayer.indexOf('<li id='))}<li>со счтеом:${bestScore}!</li>`;
    }
    ...
    if (event.data.includes('word')) {
     ...
     //заводим люйдей в таблицу счета ставим ограничение на добавление:   
        if(!scoreBar.querySelector(`.${msg.username}`)){
            messageContainer.innerHTML += `${msg.username}<li>угадал!</li>`;
            ...
            messageContainer.innerHTML += `${msg.username}<li>угадал!</li>`;
            let score =  +scoreBar.querySelector(`.${msg.username}`).querySelector('li').innerHTML
